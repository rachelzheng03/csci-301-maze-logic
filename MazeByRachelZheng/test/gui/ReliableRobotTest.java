...
class ReliableRobotTest {
	...
	public void testAddDistanceSensor(){
		...
		ReliableSensor testSensor = new ReliableSensor();
		robot.addDistanceSensor(testSensor, Direction.FORWARD);
		assertNotNull(testSensor);
		assertEquals(Direction.FORWARD, testSensor.mountedDirection);
		assertEquals(maze, testSensor.maze);
	...
	public void testStartFailureAndRepairProcess() {
		...
		try {
			robot.startFailureAndRepairProcess(Direction.FORWARD, 0, 0);
			fail("method not supported for reliable robot");
		} catch (Exception e) {
			// TODO: handle exception
			assertTrue(e instanceof UnsupportedOperationException);
			assertTrue(e.getMessage()=="method not supported for reliable robot");
		}
